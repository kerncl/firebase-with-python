...
# stocks_list = get_stocks_info(country_code, user_agent) # Return information of stock list which is websraping from web
# stocks_list.head(5)
...
# Scrap the historical prices of stocks : [0]: historical prices, sector, stock code
# historical_data = list(map(lambda x: historical_extract(x,start_date,end_date,user_agent), stocks_list['price_weblink'].values))  #LK: dump into excel file
...
# Change Nan, negative value to zero # LK
stocks_list['sales_growth_5yr(%)'] = stocks_list['sales_growth_5yr(%)'].fillna(0)
stocks_list['eps_growth_5yr(%)'] = stocks_list['eps_growth_5yr(%)'].fillna(0)
stocks_list['sales_growth_5yr(%)'] = stocks_list['sales_growth_5yr(%)'].clip(lower=0)
stocks_list['eps_growth_5yr(%)'] = stocks_list['eps_growth_5yr(%)'].clip(lower=0)
...
iplot({
    "data": [go.Scatter(x= stocks_list['sd_log_returns'], y= stocks_list['mean_log_returns'], showlegend= True,
                        mode = 'markers', marker=dict(size=(stocks_list['sales_growth_5yr(%)'])*(1/2),
                                                      ...
                                                      colorbar=dict(title='EPS growth(%) over last 5 years', titlefont=dict(size=10)), colorscale='blues',showscale=True),
...
# Replace Nan, negative with zero   #LK
stocks_tp['sales_TTM_change_1yr(%)'] = stocks_tp['sales_TTM_change_1yr(%)'].fillna(0)
stocks_tp['netprofit_margin_TTM(%)'] = stocks_tp['netprofit_margin_TTM(%)'].fillna(0)
stocks_tp['sales_TTM_change_1yr(%)'] = stocks_tp['sales_TTM_change_1yr(%)'].clip(lower=0)
stocks_tp['netprofit_margin_TTM(%)'] = stocks_tp['netprofit_margin_TTM(%)'].clip(lower=0)
plotly.offline.plot({
...
stock_picked1 = 'TIME Dotcom Bhd'
stock_picked2 = 'Supermax Corporation Bhd'

stock_price1 = stocks_tp.loc[list(stocks_tp['name']).index(stock_picked1),'historical_price'].copy()
stock_price2 = stocks_tp.loc[list(stocks_tp['name']).index(stock_picked2),'historical_price'].copy()
...
start1 = stock_price1['Date'].iloc[0]
end1 = stock_price1['Date'].iloc[-1]
start2 = stock_price2['Date'].iloc[0]
end2 = stock_price2['Date'].iloc[-1]

growth_past_stock1 = (stock_price1['Date'].iloc[-1].year - stock_price1['Date'].iloc[0].year) / stock_price1['Date'].iloc[0].year
growth_past_stock2 = (stock_price2['Date'].iloc[-1].year - stock_price2['Date'].iloc[0].year) / stock_price2['Date'].iloc[0].year

day1 = (stock_price1['Date'].iloc[-1] - stock_price1['Date'].iloc[0]).days
day2 = (stock_price2['Date'].iloc[-1] - stock_price2['Date'].iloc[0]).days

CAGR1 = ((stock_price1['Date'].iloc[-1].year / stock_price1['Date'].iloc[0].year) **(1/(day1/tradeday_per_year))) - 1
CAGR2 = ((stock_price2['Date'].iloc[-1].year / stock_price2['Date'].iloc[0].year) **(1/(day2/tradeday_per_year))) - 1
